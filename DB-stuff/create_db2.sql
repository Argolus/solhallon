...
CREATE INDEX measurement_device_id_idx ON measurement(device_id);
...
device_id integer not null references device,
state varchar,
last_statechange timestamp(0) DEFAULT NOW()
);


GRANT INSERT,UPDATE ON state TO solar;
...
ALTER TABLE devloc ADD UNIQUE (location_id,device_id,enumeration);